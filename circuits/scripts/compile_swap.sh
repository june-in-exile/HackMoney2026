...
node scripts/generateSwapTestInput.js

# Step 8: Generate witness and proof
echo "[8/12] Generating witness and proof..."
snarkjs groth16 fullprove \
    ${BUILD_DIR}/swap_input.json \
    ${BUILD_DIR}/swap_js/swap.wasm \
    ${BUILD_DIR}/swap_final.zkey \
    ${BUILD_DIR}/swap_proof.json \
    ${BUILD_DIR}/swap_public.json

# Step 9: Verify the proof
echo "[9/12] Verifying proof..."
snarkjs groth16 verify \
    ${BUILD_DIR}/swap_vk.json \
    ${BUILD_DIR}/swap_public.json \
    ${BUILD_DIR}/swap_proof.json

# Step 10: Convert VK to Sui format
echo "[10/12] Converting VK to Sui format..."
node scripts/arkworksConverterSwap.js
...
echo "      cd ../../contracts/scripts && ./update_swap_vk.sh"