...
if [ ! -f "$PTAU_FILE" ]; then
    ...
    echo "Downloading powersOfTau28_hez_final_14.ptau (~200MB)..."
...
node scripts/generateUnshieldTestInput.js

# Step 8: Generate witness and proof
echo "[8/12] Generating witness and proof..."
snarkjs groth16 fullprove \
    ${BUILD_DIR}/unshield_input.json \
    ${BUILD_DIR}/unshield_js/unshield.wasm \
    ${BUILD_DIR}/unshield_final.zkey \
    ${BUILD_DIR}/unshield_proof.json \
    ${BUILD_DIR}/unshield_public.json

# Step 9: Verify the proof
echo "[9/12] Verifying proof..."
snarkjs groth16 verify \
    ${BUILD_DIR}/unshield_vk.json \
    ${BUILD_DIR}/unshield_public.json \
    ${BUILD_DIR}/unshield_proof.json

# Step 10: Convert VK to Sui format
echo "[10/12] Converting VK to Sui format..."
node scripts/arkworksConverterUnshield.js
...
echo "      cd ../../contracts/scripts && ./update_unshield_vk.sh"