import { Transaction } from "@mysten/sui/transactions";
import { getFullnodeUrl, SuiClient } from "@mysten/sui/client";
import { fromHex } from "@mysten/sui/utils";
import { execSync } from "child_process";
import fs from "fs";

const PACKAGE_ID = "0xb2ab082080abf37b3e0a1130db3f656eba53c7aa6e847ae3f9d1d5112248a080";
const SUI_COIN_TYPE = "0x2::sui::SUI";
const VK_HEX = "";

const activeAddress = execSync("sui client active-address", { encoding: "utf-8" }).trim();
const client = new SuiClient({ url: getFullnodeUrl("testnet") });

console.log("Creating pool...");
const tx = new Transaction();
tx.setSender(activeAddress);

const vkBytes = Array.from(fromHex(VK_HEX));
tx.moveCall({
  target: `${PACKAGE_ID}::pool::create_shared_pool`,
  typeArguments: [SUI_COIN_TYPE],
  arguments: [tx.pure.vector("u8", vkBytes)],
});

tx.setGasBudget(100_000_000);

const txBytes = await tx.build({ client });
fs.writeFileSync("/tmp/pool_tx.txt", Buffer.from(txBytes).toString("base64"));
console.log("Transaction saved to /tmp/pool_tx.txt");
console.log("\nRun: sui client execute-signed-tx --tx-bytes $(cat /tmp/pool_tx.txt)");
